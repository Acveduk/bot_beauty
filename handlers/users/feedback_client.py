...
from states.state_users import Feedback_client
from utils.db_api import postgres


@dp.callback_query_handler(text_contains='feedback_client')
async def feedback(call: types.CallbackQuery, state: FSMContext):
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    keyboard.row('Отмена')
    await bot.send_message(chat_id=call.from_user.id, text=f"Введите ID клиента:\n"
                                                           f"Например: ID:1", reply_markup=keyboard)
    await Feedback_client.client.set()


@dp.message_handler(text='Отмена', state=Feedback_client.client)
async def feedback(call: types.CallbackQuery, state: FSMContext):
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
...
@dp.message_handler(state=Feedback_client.client)
async def feedback(message: types.Message, state: FSMContext):
    id_user = message.text.split(":")[-1]
    id_user = int(id_user)
    client = await postgres.get_client(user_number=id_user)
    if client != []:
        if client[0]['manicure_pedicure'] and client[0]['hairdresser'] and client[0]['visage'] and client[0]['other_services'] != 'Пусто':
            keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
            keyboard.row('Отмена')
            ...
            await bot.send_message(chat_id=message.from_user.id,
                                   text=f"Подготовка поля отзыва о клиенте: {client[0]['name_user']}",
                                   reply_markup=keyboard2)
            await bot.send_message(chat_id=message.from_user.id,
                                   text=f"1/3. Поставьте звезды пользователю",
                                   reply_markup=keyboard)
            await Feedback_client.star.set()
    else:
        keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
        keyboard.row('Отмена')
        await bot.send_message(chat_id=message.from_user.id,
...
@dp.message_handler(text='Отмена', state=Feedback_client.star)
async def feedback(call: types.CallbackQuery, state: FSMContext):
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    keyboard.row('❤️Избранное', '📝 Создать заявку на услугу')
    keyboard.row('🏅 Найти мастера по рейтингу', '💬 Канал/Чат')
...
@dp.callback_query_handler(text=['one_star', 'two_star', 'three_star', 'four_star', 'five_star'], state=[Feedback_client.star, Feedback_client.feedback])
async def feedback(call: types.CallbackQuery, state: FSMContext):
    if call.data == 'one_star':
        ...
        await Feedback_client.feedback.set()
...
@dp.callback_query_handler(text='сonfirm', state=Feedback_client.feedback)
async def feedback(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(text='3/3. Напишите отзыв клиенту')
    await Feedback_client.recall.set()


@dp.message_handler(text='Отмена', state=Feedback_client.feedback)
async def feedback(call: types.CallbackQuery, state: FSMContext):
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    keyboard.row('❤️Избранное', '📝 Создать заявку на услугу')
    keyboard.row('🏅 Найти мастера по рейтингу', '💬 Канал/Чат')
...
@dp.message_handler(text='Отмена', state=Feedback_client.recall)
async def feedback(call: types.CallbackQuery, state: FSMContext):
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    keyboard.row('❤️Избранное', '📝 Создать заявку на услугу')
    keyboard.row('🏅 Найти мастера по рейтингу', '💬 Канал/Чат')
...
@dp.message_handler(state=Feedback_client.recall)
async def feedback_client(message: types.Message, state: FSMContext):
    ...
    if await postgres.check_recall_master(user_id=user_id):
        ...
        with open(f'feedback_client/{user_id}.txt', 'a', encoding='utf-8') as file:
        ...
        await postgres.add_recall_master(user_id=user_id, recall=recall, name_recall=name_recall)
        with open(f'feedback_client/{user_id}.txt', 'w', encoding='utf-8') as file:
            file.write(f'Отзыв оставил(а): {name_recall}\n'
                       f'Отзыв: {recall}\n\n')
